...
func (r *RedisRepo) FindByID(ctx context.Context, id uint64) (model.Order, error) {
	...
	if errors.Is(err, redis.Nil) {
		return model.Order{}, ErrNotExist // much cheaper to send an empty object rather than a pointer for the success case, because it allocates on the heapj
	} else if err != nil {
	...
	if err != nil {
		return model.Order{}, fmt.Errorf("failure to decode order: %w", err)
	}
